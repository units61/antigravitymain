...
def get_directive() -> str:
    ...
    directive_path = Path(__file__).parent.parent / "skills" / "design-dna-extractor" / "SKILL.md"
    if not directive_path.exists():
        # Fallback if accessed from other paths
        directive_path = Path(__file__).parent / "directives" / "prompt-understanding.md"
    if not directive_path.exists():
        raise FileNotFoundError(f"Directive prompt-understanding not found.")
